...
- 插件先將清理後正文行尾精確對應原文，提供 `p1` 等位置編號；LLM 回傳資料型 JSON（`scenes[].after_id / label / prompt`），不需逐字抄寫 Markdown 段尾。編號驗證通過後才插入 `[[cmi-image:ID]]`。**不讓 LLM 改寫原文，也不將回覆當成 HTML／JS 執行。** 舊 `after` 摘錄格式仍採嚴格唯一匹配。無效／重複編號、非 JSON 或數量超限時整次拒絕，不自動重試；排除程式碼、引用及 HTML 屬性／註解／raw-text 內位置。
- 沿用所選預設的順序、樣式、角色、隔離巨集及輸入 raw／prompt 正則；最後附加規劃格式指令與同樣經過過濾的目標正文。**插圖 JSON 不再套用正文清理或預設回覆 raw／prompt／display 正則**，避免有效計畫被一般角色對話清理規則刪空。只容許移除完整的開頭 think／thinking 區塊與外層 JSON fence；一般獨立提示詞流程不變。此操作不開啟預設 HTML 互動面板，模型本身仍須能回傳有效 JSON。
- 沒有能精確對應原文的安全位置時，在 LLM 請求前停止；不模糊匹配或退回未清理正文。重寫／重排文字不保證可定位，宜使用保留正文原字句的清理規則。同步定位掃描限制清理前後正文合計 20 萬字元／2000 行，超限需縮短樓層。
