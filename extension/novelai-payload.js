...
// Kept equivalent to the v2 backend, including all six models and safety limits.
...
export const SAMPLERS = Object.freeze(['k_euler_ancestral', 'k_euler', 'k_dpmpp_2m', 'k_dpmpp_sde', 'k_dpmpp_2s_ancestral']);
export const SCHEDULES = Object.freeze(['native', 'karras', 'exponential', 'polyexponential']);
...
export function buildNovelAIRequest(payload) {
    ...
    if (seed === undefined || seed === '' || seed === -1 || seed === '-1') seed = crypto.getRandomValues(new Uint32Array(1))[0];
    else if (typeof seed === 'string' && /^\d{1,10}$/.test(seed)) seed = Number(seed);
    seed = numeric(seed, 0, 'seed', 0, 0xffffffff, true);
    const parameters = {
        params_version: 3, width, height, n_samples: samples, seed,
        ...
        negative_prompt: negative, image_format: 'png', qualityToggle: false, ucPreset: 0,
        prefer_brownian: true, deliberate_euler_ancestral_bug: false, dynamic_thresholding: false,
        legacy: false, legacy_v3_extend: false, sm: false, sm_dyn: false, add_original_image: false,
    };
    if (model.startsWith('nai-diffusion-4')) {
        parameters.v4_prompt = { caption: { base_caption: payload.prompt, char_captions: [] }, use_coords: false, use_order: true };
        parameters.v4_negative_prompt = { caption: { base_caption: negative, char_captions: [] } };
    }
